      ...
      <section className="w-full bg-[#0A0A0A] py-14 lg:py-24">
        <div className="container mx-auto px-4 max-w-[1280px] text-center">
          ...
          <h1 className="text-white font-serif font-bold text-[32px] md:text-[48px] lg:text-[64px] leading-[1.1] mb-5 lg:mb-6">
            Contact{" "}
            <span className="italic text-[var(--color-primary)]">{storeName}</span>
          </h1>
        ...
        <div className="container mx-auto px-4 max-w-[1280px]">
          <div className="text-center mb-8 lg:mb-14">
            ...
            <h2 className="text-2xl lg:text-[40px] font-serif">
              ...
              <span className="italic text-[var(--color-primary)]">{ct.channel}</span>
            </h2>
          </div>
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 gap-6 ${channels.length === 4 ? "lg:grid-cols-4" : channels.length === 3 ? "lg:grid-cols-3" : "lg:grid-cols-2"}`}>
            {channels.map((c) => (
              <a
                ...
                className="group bg-white border border-gray-200 rounded-2xl p-8 flex flex-col items-center text-center hover:border-[var(--color-primary)] hover:shadow-lg transition-all duration-200"
                style={{ boxShadow: "0 4px 16px rgba(0,0,0,0.04)" }}
              >
                <div className="w-14 h-14 rounded-xl border border-[var(--color-primary)] bg-[#FFF8F8] flex items-center justify-center mb-5 group-hover:bg-[var(--color-primary)] transition-colors duration-200 [&_svg]:group-hover:stroke-white">
                  {c.icon}
                </div>
